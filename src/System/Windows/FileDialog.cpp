#include "PIIK/Platform/Windows/FileDialog.h"
...
namespace Piik::Windows
{
    ...
    static FileTypeFilterW GetWideStringVersion(const Dialog::FileTypeFilter& fileFilter)
    {
    ...
    std::optional<std::string> Dialog::ShowShellDialogAndGetResult(IFileDialog* pFileDialog)
    {
    ...
    std::optional<std::string> Dialog::OpenFile(const std::string& titleMsg, const std::vector<FileTypeFilter>* pFilter)
    {
    ...
    std::optional<std::string> Dialog::SaveFile(const std::string& titleMsg, const std::string& defaultName,
        const std::vector<FileTypeFilter>* pFilter)
    {
        if (FAILED(CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED | COINIT_DISABLE_OLE1DDE)))
            return std::nullopt;
    ...
    std::optional<std::string> Dialog::OpenDirectory(const std::string& titleMsg)
    {
        if (FAILED(CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED | COINIT_DISABLE_OLE1DDE)))
            return std::nullopt;
